...
character(400), dimension(:,:),allocatable :: smtal
character(400), dimension(:),allocatable :: smtalaux
real*4 :: x0
real*4 :: y0
integer :: nuclint
real*4 :: movx, movy
...
open(999,file='dynamics_points.tmp')
read(999,*) npoints
close(999)


allocate(smtal(ntally,npoints))
allocate(smtalaux(ntally*npoints))

!************iterate for more tallies***************
!k=1
open(888,file='sumtally.tmp')
do i=1,ntally*npoints
read(888,'(A)') smtalaux(i)
end do
close(888)
!***************************
...
do j=1,npoints
!print*, k
smtal(i,j) = smtalaux(k)
k = k + ntally
!print*, trim(smtal(i,j))
end do
...
open(806,file='px.tmp')
read(806,*) px
close(806)

open(807,file='py.tmp')
read(807,*) py
close(807)
...
open(808,file='sx.tmp')
read(808,*) sx
close(808)

open(809,file='sy.tmp')
read(809,*) sy
close(809)

open(810,file='sz.tmp')
read(810,*) sz
close(810)
...
call system("./shieldmat.exe")

open(777,file='linecounts.tmp')
read(777,*) nshield
close(777)

allocate(shieldmat(nshield)) 
allocate(shieldfrac(nshield))

open(555,file='shx.tmp')
read(555,*) shx
close(555)

open(555,file='shl.tmp')
read(555,*) shl
close(555)

open(555,file='shy.tmp')
read(555,*) shy
close(555)

open(555,file='shw.tmp')
read(555,*) shw
close(555)

open(555,file='shz.tmp')
read(555,*) shz
close(555)


open(555,file='shieldmat.tmp')

do i =1,nshield


read(555,*) shieldmat(i), shieldfrac(i)

end do

close(555)

open(555,file='shieldrho.tmp')
read(555,*) shieldrho
close(555)
...
write(10,19) adjustl(trim("mat[18]        $Shield"))

do i=1,nshield
write(10,559) adjustl(trim(shieldmat(i))),trim(shieldfrac(i))
559 format(A,X,A)
end do
!***************************************************************here shield mat needs to read in array!
...
135 format(A,X,F8.4,X,F8.4,X,F8.4,X,F8.4,X,A,X,F8.4)
...
write(10,448) adjustl(trim("file = organ_deposit_")),trim(tally(i)),trim(".out")
!838 format(3X,A,A,A)
...
write(10,38) adjustl(trim("sumtally start"))

write(10,38) adjustl(trim("isumtally = 2"))

write(10,668) adjustl(trim("nfile = ")),npoints !change to dynamic
668 format(3X,A,I3)
!*******must be changed to dynamic
do j=1,npoints
write(10,738) adjustl(trim(smtal(i,j))), " 1.0"
!*******
end do
write(10,448) adjustl(trim("sfile = organ_deposit_sumtally_")),trim(tally(i)),trim(".out")
448 format(3X,A,A,A)
write(10,38) adjustl(trim("sumfactor = 1.0"))

write(10,38) adjustl(trim("sumtally end"))
...
write(10,668) adjustl(trim("nfile = ")),npoints
...
if(shield==1)then
deallocate(shieldmat) 
deallocate(shieldfrac)
end if
